...
export default function SentenceForm({ initialValues, onSubmit }:{initialValues:Values;onSubmit:(v:Values)=>void}){
  const roles=['Competitive Intelligence Lead','VP of Strategy'];
  const industries=['FinTech','Healthcare'];
  const [role,setRole]=useState(initialValues.role);
  const [industry,setIndustry]=useState(initialValues.industry);
  return(
    <form onSubmit={e=>{e.preventDefault();onSubmit({role,industry})}} className="border border-white/20 rounded-xl p-6 space-y-6 backdrop-blur-md bg-black/40">
      <p className="text-lg">
        I am a&nbsp;
